...
- Below is the directory structure of the dbt-assignment-1-repo project:
...
│   ├── get_total_revenue.sql
│   ├── incremental_config.sql
│   └── to_date.sql
├── models/
│   └── ecommerce/
│       ├── analytics/
...
│       │   ├── rfm_scores.sql
│       │   ├── rfm_segment.sql
│       │   └── sales_with_growth.sql
│       ├── cleaning/
│       │   └── clean_retail_data.sql
│       └── staging/
│           ├── incremental_raw_retail_data.sql
│           └── stg_raw_retail_data.sql
├── seeds/
├── snapshots/
│   └── raw_retail_data_snapshot.sql
├── tests/
├── .gitignore
├── README.md
└── dbt_project.yml
...
## Macro: filter_conditions

The filter_conditions macro is a reusable piece of logic that allows filtering conditions to be dynamically applied in SQL queries. It helps standardize filtering criteria for customer transactions, ensuring data consistency across models.

### **Usage**
This macro is used to apply conditional filters based on:
1. **Excluding NULL customer IDs** (optional).
2. **Filtering records based on a minimum quantity threshold**.
...
The incremental_config macro is a reusable configuration macro in dbt that simplifies the process of defining **incremental models**. This macro ensures that models are **materialized as incremental tables** and helps handle deduplication using a unique key.

### **Purpose**
- Helps standardize the configuration of incremental models.
- Avoids redundancy by using a macro instead of repeating configuration settings in multiple models.
- Ensures **Merge strategy** for incremental loads, which updates existing rows when the unique_key matches.
...
The get_total_revenue macro is a simple dbt macro designed to calculate the **total revenue** by multiplying the **quantity** of items sold by their respective **price**.

### **Purpose**
- Simplifies the calculation of total revenue across multiple models.
- Reduces redundancy by allowing dynamic column references instead of hardcoding calculations.
- Ensures consistency in revenue computation throughout the project.
